...
function Script11()
{
  ...
  // --- muat Firebase compat SDK & auth anon (sekali) ---
    ...
    add('https://www.gstatic.com/firebasejs/10.12.2/firebase-app-compat.js', function(){
      add('https://www.gstatic.com/firebasejs/10.12.2/firebase-auth-compat.js', function(){
        add('https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore-compat.js', function(){
          ...
          firebase.auth().signInAnonymously().then(function(){ window._fbReady = true; cb(); })
  ...
  // --- nama → key dokumen ---
  var name = (p.GetVar('vNama')||'').trim();
  if (!name){ /* biarkan layer notifikasi kamu yang menangani */ }
  var key  = name.toLowerCase();
  p.SetVar('vNameKey', key);

  // --- daftar variabel yang disimpan ---
  var boolVars = ['vMateri1','vMateri2','vMateri2a','vMateri2c','vMateri3','vMateri4','vMateri5'];
  var textVars = [
    'vTabelDiskriminan11','vTabelDiskriminan12','vTabelDiskriminan21','vTabelDiskriminan22','vTabelDiskriminan31','vTabelDiskriminan32',
    'vTabelKoefisienA11','vTabelKoefisienA12','vTabelKoefisienA21','vTabelKoefisienA22','vTabelKoefisienA31','vTabelKoefisienA32','vTabelKoefisienA41','vTabelKoefisienA42',
  ...
  var defaultState = {}; boolVars.forEach(k=>defaultState[k]=false); textVars.forEach(k=>defaultState[k]='');

  function applyState(st){
    boolVars.forEach(k=>p.SetVar(k, !!st[k]));
    textVars.forEach(k=>p.SetVar(k, (st[k]!=null? String(st[k]) : '')));
  }

  // --- LOAD/CREATE di Firestore ---
  ensureFirebase(function(){
    ...
    ref.get().then(function(doc){
      if (doc.exists){
        var st = (doc.data() && doc.data().state) || defaultState;
        applyState(st);
        try{ localStorage.setItem('state_'+key, JSON.stringify(st)); }catch(e){}
      }else{
        ref.set({ name:name, state:defaultState, updated: firebase.firestore.FieldValue.serverTimestamp() });
        applyState(defaultState);
        try{ localStorage.setItem('state_'+key, JSON.stringify(defaultState)); }catch(e){}
      }
      p.SetVar('vLoaded', true); // sinyal siap pindah
    }).catch(function(){
      applyState(defaultState);
